---
title: Typing FAQ
description: Frequently asked questions about ty and Python's type system
---

This page answers commonly asked questions about ty and Python's type system.

<Accordion title="Why does ty report an error on my code?">

Check the [rules reference](/reference/rules) for the specific error code you are seeing; it may explain the problem.

</Accordion>

<Accordion title="What is the Unknown type and when does it appear?">
...
ty also uses unions with `Unknown` to maintain the [gradual guarantee](../features/type-system.md#gradual-guarantee), which helps avoid false positive errors in untyped code while still providing useful type information where possible.
...
[Try this example in the playground](https://play.ty.dev/862941a8-a3f6-4818-9ea1-d9d59b0bd2fa)

</Accordion>

<Accordion title="Why does ty show int | float when I annotate something as float?">
...
[Try this example in the playground](https://play.ty.dev/fb034780-3ba7-4c6a-9449-5b0f44128bab)
...
<Accordion title="Why does ty say Callable has no attribute __name__?">
...
[Try various approaches in the playground](https://play.ty.dev/f6f7f35a-47c3-423d-be8d-33d03c61d40c)
...
[Check out the full example](https://play.ty.dev/7a1ea4ab-04e1-4271-adf5-ddc3a5d2fcfd)
</Info>

</Accordion>

<Accordion title="Does ty have a strict mode?">
...
<Accordion title="Why doesn't ty warn about missing type annotations?">

ty does not report an error for unannotated function parameters, return types, or variables. When ty encounters an unannotated symbol, it infers the type as `Unknown` while still providing useful diagnostics where possible.
...
<Accordion title="Why can't ty resolve my imports?">
...
1. **Virtual environment**: Make sure your virtual environment is discoverable. ty looks for an active virtual environment via `VIRTUAL_ENV` or a `.venv` directory in your project root. See the [module discovery](../modules.md#python-environment) documentation for more details.

2. **Project structure**: If your source code is not in the project root or `src/` directory, configure [`environment.root`](./configuration.md#root) in your `pyproject.toml`:
...
4. **Compiled extensions**: ty requires `.py` or `.pyi` files for type information. If a package contains only compiled extensions (`.so` or `.pyd` files), you'll need stub files (`.pyi`) for ty to understand the types.

</Accordion>

<Accordion title="Does ty support monorepos?">
...
2. **Configure multiple source roots**: Use [`environment.root`](./configuration.md#root) to specify multiple source directories:
...
<Accordion title="Does ty support PEP 723 inline-metadata scripts?">
...
<Accordion title="Is there a pre-commit hook for ty?">
...
<Accordion title="Does ty support (mypy) plugins?">
...
<Accordion title="What is Top[list[Unknown]], and why does it appear?">
...
See also the [discussion here](/concepts/type-system#top-and-bottom-materializations) and [in this issue](https://github.com/astral-sh/ty/issues/1578).
